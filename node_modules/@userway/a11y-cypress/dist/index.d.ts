...
export interface Issue<T extends TargetElement = Element> {
	...
	element: T;
	elementId: string;
	encodedContent?: number[] | undefined;
...
export type Rule<T extends TargetElement = TargetElement> = {
	...
	description: RuleDescriptor["description"];
	display: RuleDescriptor["display"];
	errorMessage: RuleDescriptor["errorMessage"];
	id: Base["id"];
	level: RuleDescriptor["level"];
	outputType?: Base["outputType"];
	recommendation: RuleDescriptor["recommendation"];
	references: RuleDescriptor["references"];
	severity: RuleDescriptor["severity"];
	areas: RuleDescriptor["areas"];
	disabilitiesAffected: RuleDescriptor["disabilitiesAffected"];
	successCriteria: RuleDescriptor["successCriteria"];
	uwId: Base["uwId"];
};
...
export interface AlchemyFix {
	fixType?: number;
	domSpec?: boolean;
	helperText?: string;
	fix: Record<string, string | boolean | null>;
}
...
export interface AccessEngineReportRecord {
	attribute: string;
...
export interface AnalysisConfig {
	...
	rootSelector?: string;
	onRuleError?: (error: AnalyzeError) => void;
}
export type AnalyzeResultRecord = RuleAnalyzeResult<any> & {
	...
	accessEngineReport?: AccessEngineReportRecord[];
};
...
export declare enum RulesIds {
	...
	Bypass = "bypass",
	BypassHeading = "bypass-heading",
	BypassLandmark = "bypass-landmark",
	BypassMoveFocus = "bypass-move-focus",
	...
	EventsHandling = "events-handling",
	...
	FrameValidAttrValue = "frame-valid-attr-value",
	HeadingOrder = "heading-order",
	HiddenContent = "hidden-content",
	HrValidAttr = "hr-valid-attr",
	...
	SkipLink = "skip-link",
	SuspiciousAccessibleName = "suspicious-accessible-name",
	...
	TargetSizeLarge = "target-size-large",
...
export type Framework = "Cypress" | "Playwright" | "Puppeteer" | "UserWay" | "Selenium Python" | "Selenium Java" | "Selenium C#" | "WebdriverIO" | "Continuum Appium";
export type ReportFileFormat = "json" | "html" | "csv";
export type AnalysisMode = "background" | "manual";
export type ScreenshotsMetadata = Record<string, Record<string, string>>;
...
export type LaunchResultData = {
	...
	fullReport: Record<string, AnalyzeResultRecord>;
	analyzeErrors: AnalyzeError[];
	analyzeConfig?: AnalysisConfig;
};
export type AnalyzeConfig = LaunchConfig & {
	...
	savePageOrigin?: boolean;
};
export declare const setupUserway: (configOverride: AnalyzeConfig) => void;
declare function analyze(config?: Omit<AnalyzeConfig, "reportPath" | "onRuleError">): Cypress.Chainable<{
	result: LaunchResultData;
	report: {
		readonly screenshotsMeta: {};
		readonly format: "json";
		readonly report: AnalyzeResultRecord[];
		readonly meta: {
			mode: AnalysisMode;
			customTags: string[];
			date: string;
		...
		readonly errors: AnalyzeError[];
		readonly savePageOrigin: boolean;
		readonly config: {
			...
			level?: "A" | "AA" | "AAA" | null | undefined;
			ignoreSelectors?: IgnoreElements | undefined;
			switchOff?: boolean | undefined;
			ruleSpecificOptions?: RuleSpecificOptions | undefined;
			rootSelector?: string | undefined;
			onRuleError?: ((error: AnalyzeError) => void) | undefined;
			readonly reportIncomplete?: boolean | undefined;
			readonly customTags?: string[] | undefined;
			readonly strict?: boolean | undefined;
			readonly elementScreenshots?: boolean | undefined;
			readonly pageScreenshot?: boolean | undefined;
			readonly reportPath?: string | undefined;
			readonly ignoreUrls?: RegExp[] | undefined;
			readonly onResult?: ((data: LaunchResultData) => void | Cypress.Chainable<unknown>) | undefined;
			readonly savePageOrigin?: boolean | undefined;
			readonly mode: AnalysisMode;
		};
	};
}>;
export interface SaveReportConfig {
	report: Array<AnalyzeResultRecord>;
	meta: AnalysisReportMeta;
...
declare function saveReport({ report: body, errors, format, reportPath, screenshotsMeta, savePageOrigin, pageUrl, meta, config, }: SaveReportConfig): Cypress.Chainable<null>;
declare function start(config?: AnalyzeConfig): null;
declare function stop$1(): Cypress.Chainable<null> | Cypress.Chainable<Cypress.Chainable<{
	result: LaunchResultData;
	report: {
		readonly screenshotsMeta: {};
		readonly format: "json";
...
declare global {
	export namespace Cypress {
		interface Chainable {
			...
			userwayStart: typeof start;
			userwayStop: typeof stop$1;
		}
	}
}
...
export type AnalyzeParams = Parameters<AnalyzeFn>;
export type SaveReportParams = Parameters<SaveReportFn>;
export type SetupUserwayParams = Parameters<SetupUserwayFn>;
