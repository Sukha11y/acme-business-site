...
declare enum ContinuousAccessibilityApiErrorType {
    ...
    UNSUPPORTED_INTEGRATION_ERROR = "UNSUPPORTED_INTEGRATION_ERROR",
    UNSUPPORTED_ALM_NAMESPACE_TYPE_ERROR = "UNSUPPORTED_ALM_NAMESPACE_TYPE_ERROR",
    UNSUPPORTED_PROJECT_TYPE_ERROR = "UNSUPPORTED_PROJECT_TYPE_ERROR",
    ...
    ONLY_OWNER_OR_ORGANIZATION_ADMIN_CAN_MODIFY_COMMENT_ERROR = "ONLY_OWNER_OR_ORGANIZATION_ADMIN_CAN_MODIFY_COMMENT_ERROR",
    ...
    USER_IS_ALREADY_MEMBER_ERROR = "USER_IS_ALREADY_MEMBER_ERROR",
    INVALID_PROJECT_CONNECTION_ERROR = "INVALID_PROJECT_CONNECTION_ERROR",
    PROJECT_IS_ALREADY_CONNECTED_ERROR = "PROJECT_IS_ALREADY_CONNECTED_ERROR",
    INVALID_REPOSITORY_CONNECTION_ERROR = "INVALID_REPOSITORY_CONNECTION_ERROR",
    UNKNOWN_ERROR = "UNKNOWN_ERROR",
    ALM_INTEGRATION_NETWORK_EXCEPTION = "ALM_INTEGRATION_NETWORK_EXCEPTION",
    ALM_INTEGRATION_INVALID_REQUEST_EXCEPTION = "ALM_INTEGRATION_INVALID_REQUEST_EXCEPTION",
    ALM_INTEGRATION_AUTHORIZATION_EXCEPTION = "ALM_INTEGRATION_AUTHORIZATION_EXCEPTION",
    INVALID_QUALITY_PROFILE_EXCEPTION = "INVALID_QUALITY_PROFILE_EXCEPTION",
    ALM_AI_CODE_REMEDIATION_EXCEPTION = "ALM_AI_CODE_REMEDIATION_EXCEPTION"
}
...
interface CreateProject<Connection extends IntegrationConnection> {
    name: string;
...
interface GitIntegrationConnection extends IntegrationConnection {
    integration: Integration.GIT;
    repository: GitRepository;
}
interface AlmIntegrationConnection extends IntegrationConnection {
    integration: Integration.BITBUCKET | Integration.GITHUB | Integration.GITLAB | Integration.AZURE;
    repository: AlmIntegrationRepository;
}
...
declare enum AlmIntegrationNamespaceType {
    USER = "USER",
    TEAM = "TEAM"
}
interface AlmIntegrationNamespace {
    externalId: string;
    name: string;
    slug: string;
    type: AlmIntegrationNamespaceType;
}
interface AlmIntegrationRepositoryResponse {
    externalId: string;
    name: string;
    slug: string;
    integration: Integration;
    namespace: AlmIntegrationNamespace;
    mainBranch: string | undefined;
    isPrivate: boolean;
    isImported: boolean;
}
interface AlmIntegrationRepository {
    externalId: string;
    name: string;
    slug: string;
    integration: Integration;
...
interface AlmIntegrationPullRequest {
    externalId: string;
    title: string;
    author: {
        name: string;
...
declare enum AlmAnnotationState {
    PENDING = "PENDING",
    SUCCESS = "SUCCESS",
    FAILURE = "FAILURE"
}
declare enum AlmAnnotationReference {
    COMMIT = "COMMIT",
...
interface RepositoryExistsQuery {
...
interface CommitScanReportContext {
    project: {
        name: string;
    };
    organization: {
...
declare enum CommitScanReportStatus {
    SUCCESS = "SUCCESS",
    FAILURE = "FAILURE"
}
interface ReadyScanReport {
    instanceId: string;
...
interface AzureCommitScanReport {
    report: string;
}
type CommitScanReport = {
    context: CommitScanReportContext;
} & ({
    status: CommitScanReportStatus.SUCCESS;
    value: ReadyScanReport;
} | {
    status: CommitScanReportStatus.FAILURE;
    error: ContinuousAccessibilityApiErrorType;
});
interface CommitScanReportsList {
    reports: CommitScanReport[];
}
interface MainBranchCommitScanReportsParams {
    integration: Integration;
    namespaceType: AlmIntegrationNamespaceType;
    namespaceExternalId: string;
    repositoryExternalId: string;
}
interface PullRequestCommitScanReportsParams {
    integration: Integration;
    namespaceType: AlmIntegrationNamespaceType;
    namespaceExternalId: string;
    repositoryExternalId: string;
...
interface AzureCommitScanReportParams {
    commitScanId: string;
}
...
declare enum TestFramework {
    ...
    CONTINUUM_APPIUM = "Continuum Appium"
}
interface Scan {
    id: string;
...
interface IssueDocumentFacets {
    ...
    testFrameworks: Record<TestFramework, number>;
...
interface Ca11yUpstreamErrorResponse {
    requestUrl?: string;
    statusCode: number;
    serviceResponse: Ca11yServiceErrorResponseDto;
}
type ApiErrorType = string;
interface Ca11yErrorResponseElement {
    errorClass?: string;
    ...
    upstreamError?: Ca11yUpstreamErrorResponse;
}
interface Ca11yServiceErrorResponseDto {
    errors: Ca11yErrorResponseElement[];
}
...
interface ChildIssueCodeLocation {
    cssSelector: string;
    pathSelector: string;
    xpath: string;
    snippet: string | null;
...
interface ChildIssueDetailed {
    id: string;
    location: string;
    testFrameworks: TestFramework[];
    browsers: BrowserName[];
    codeLocations: ChildIssueCodeLocation[];
}
type ChildIssueDetailedList = Pageable<ChildIssueDetailed>;
interface IssueDetailed {
    id: string;
    key: string;
    type: IssueType;
    groupKey: string | null;
    ...
    childIssues: ChildIssueDetailedList;
    createdAt: DateTime;
    updatedAt: DateTime;
}
interface ChildIssueRawPayloadElement {
    fingerprint: string;
    browser: BrowserName;
    userAgent?: string;
...
interface ChildIssueRaw {
    id: string;
    issueId: string;
    payloadElements: ChildIssueRawPayloadElement[];
    createdAt: DateTime;
}
interface IssueRaw {
    id: string;
...
interface CreateOAuthConnection {
    authCode: string;
}
...
interface ParsedAlmNamespaceParam {
    slug: string;
    type: AlmIntegrationNamespaceType;
}
interface ReconnectRepositoryParams {
    integration: Integration;
...
interface SsoUserInfo {
    ...
    defaultCa11yOrganizationSlug: string;
}
...
interface UserSsoConfiguration {
    id: string;
    registrationId: string;
    domain: string;
    protocol: SsoProtocolType;
    isActive: boolean;
    samlConfiguration: UwSamlConfiguration | null;
    oauthConfiguration: UwOAuthConfiguration | null;
    dateCreated: DateTime;
    dateModified: DateTime;
}
interface UwSamlConfiguration {
    entityId: string | null;
    metadataUriType: SamlMetadataUriType;
    metadataUrl: string | null;
    identityProvider: UwSamlProvider;
    replyUrl: string;
}
declare enum SamlMetadataUriType {
    S3_XML = "S3_XML",
    URL = "URL"
}
declare enum UwSamlProvider {
    AZURE = "AZURE",
    OKTA = "OKTA"
}
interface UwOAuthConfiguration {
    issuerUrl: string | null;
    identityProvider: UwOauthProvider;
    clientIdSuffix: string | null;
    clientSecretSuffix: string | null;
    redirectUrl: string;
    scope: string[];
}
declare enum UwOauthProvider {
    OKTA = "OKTA",
    FACEBOOK = "FACEBOOK",
    LINKEDIN = "LINKEDIN",
    IDAPTIVE = "IDAPTIVE",
...
interface EnableSsoRequest {
    ssoProtocolType: SsoProtocolType;
}
interface EnableSsoRequest {
    ssoProtocolType: SsoProtocolType;
}
interface UpdateSamlPart {
    entityId: string;
    metadataUriType: SamlMetadataUriType;
    metadataUrl: string | null;
    federationXmlBase64: string | null;
    identityProvider: UwSamlProvider;
}
interface UpdateOAuthPart {
    identityProvider: UwOauthProvider;
...
interface ChangeQualityGateQualityProfile {
...
interface QualityGateQualityProfile {
    id: string;
    name: string;
    isDefault: boolean;
}
interface QualityGateWithUsedProjectsData extends QualityGate {
    ...
    qualityProfile: QualityGateQualityProfile;
...
type QualityGateMetricBatch = Partial<Record<QualityGateMetricInitial, QualityGateConditionBriefBatch>>;
type QualityGateConditionBatch = Partial<Record<QualityGateScope, QualityGateMetricBatch>>;

declare enum QualityProfileType {
    BUILT_IN = "BUILT_IN",
    CUSTOM = "CUSTOM"
}
declare enum QualityProfileBuiltInType {
    WCAG_A = "WCAG_A",
    WCAG_AA = "WCAG_AA",
    WCAG_AAA = "WCAG_AAA",
    ALL = "ALL"
}
interface QualityProfileQualityGate {
    id: string;
    name: string;
}
declare enum QualityProfileFilterType {
    ALL = "ALL",
    LEVEL = "LEVEL",
    SEVERITY = "SEVERITY",
    KEY = "KEY"
}
interface QualityProfileFilter {
    type: QualityProfileFilterType;
    values: string[];
}
interface QualityProfileFilters {
    include: QualityProfileFilter[];
    exclude: QualityProfileFilter[];
}
interface OrganizationQualityProfile {
    id: string;
    name: string;
    description: string;
    type: QualityProfileType;
    builtInType: QualityProfileBuiltInType | null;
    isDefault: boolean;
    rulesNum: number;
    qualityGates: QualityProfileQualityGate[];
    createdAt: DateTime;
    updatedAt: DateTime;
}
...
type DocumentReindexBatch = IssueReindexDocumentBatch | ProjectReindexDocumentBatch | PageReindexDocumentBatch | AlmInstanceReindexDocumentBatch | ScanReindexDocumentBatch;
...
export { A11yTransactionType, AccessibilityScoreFilter, type AccountMfaInfoResponse, type Action, AlmAnnotationReference, AlmAnnotationState, type AlmInstance, type AlmInstanceDetails, type AlmInstanceDetailsPages, type AlmInstanceDocument, type AlmInstanceDocumentDetailed, type AlmInstanceDocumentFacets, type AlmInstanceDocumentFilter, type AlmInstanceDocumentQuery, type AlmInstanceGroupCount, type AlmInstanceList, type AlmInstanceReindexDocument, type AlmInstanceReindexDocumentBatch, AlmInstanceSortField, type AlmInstanceSortableQuery, type AlmInstanceSummaryScoreCells, AlmInstanceType, type AlmIntegrationConnection, type AlmIntegrationNamespace, AlmIntegrationNamespaceType, type AlmIntegrationPullRequest, type AlmIntegrationRepository, type AlmIntegrationRepositoryResponse, type AlmUrls, AnalysisScope, type ApiErrorType, type Artifact, ArtifactSource, ArtifactType, type AuthData, type Avatar, type AzureCommitScanReport, type AzureCommitScanReportParams, type BasicAuth, type BillingAddress, type BillingBanner, type BillingBannerResponse, BillingBannerType, type BillingEmailUser, type BillingEmailUsers, type BillingEmails, BillingMethod, type BillingOrderItem, BillingProvider, type BillingUserService, type BillingUserwaySubscription, BrowserName, type Ca11yAddon, Ca11yCustomHeader, type Ca11yCustomHeaders, type Ca11yErrorResponseElement, Ca11yGrantedMemberStatus, Ca11yMemberInvitationStatus, Ca11yMemberRole, type Ca11yServiceErrorResponseDto, Ca11yTierType, type Ca11yUpstreamErrorResponse, type CalculateCartBody, type CalculateCartPricing, type CalculateCartSubscriptionUpgradeBody, type CancelTrial, type CardDetails, type CartInvoiceTotals, type ChangeEmailBody, type ChangePasswordBody, type ChangeQualityGateQualityProfile, type ChildIssueCodeLocation, type ChildIssueDetailed, type ChildIssueDetailedList, type ChildIssueRaw, type ChildIssueRawPayloadElement, type CloseBanner, type CommitDataAlmInstance, type CommitScan, type CommitScanConfig, type CommitScanConfigPage, type CommitScanReport, type CommitScanReportContext, CommitScanReportStatus, type CommitScanReportsList, type CommitScanStatus, type ConfirmEmailMfa, type ConfirmVirtualMfa, type ConfirmVirtualMfaResponse, ContinuousAccessibilityApiErrorType, type ContributorAlmInstance, type ContributorAlmInstanceSet, type ContributorAlmInstanceWithCount, type ContributorSearch, type ContributorWithAvatar, type CoreLambdaResponse, CoreLambdaResponseStatus, type CoreLambdaResponses, type CountByOrganizationIds, type CreateOAuthConnection, type CreateProject, type CreateProjectSettings, type CreateSubscription, type CreateTopUp, type DateTime, type Delay, type Device, type DocumentReindexBatch, type DowngradeSubscriptionBody, ElasticDocumentType, type EnableSsoRequest, type FinalizeArtifactConfiguration, type FormAuth, type GenerateInviteUrlBody, type GetHelperProductsBody, type GetOrganizations, type GetPdfInvoice, type GetSubscription, type GitIntegrationConnection, type GitRepository, GrantedMemberRole, type InitializeVirtualMfa, type Instance, Integration, type IntegrationConnection, type Invitation, type InvitationParams, type InvitationResponse, InvitationStatus, type InvitationUrl, type InviteMembers, type InviteMembersResponse, type InviteSingleMember, type IsDowngradeAvailable, type IsDowngradeAvailableResponse, type Issue, type IssueActivity, type IssueActivityAssigneeChanged, type IssueActivityAssigneeChangedContext, type IssueActivityBase, type IssueActivityCommentAdded, type IssueActivityCommentAddedContext, type IssueActivityCopiedFrom, type IssueActivityCopiedFromContext, type IssueActivityCreateContext, type IssueActivityCreated, type IssueActivityDetailed, type IssueActivityFixMergedFrom, type IssueActivityFixMergedFromContext, type IssueActivityItem, type IssueActivityList, type IssueActivityMergedFrom, type IssueActivityMergedFromContext, type IssueActivityMeta, type IssueActivityTagChanged, type IssueActivityTagChangedContext, type IssueActivityTransitionChanged, type IssueActivityTransitionChangedContext, IssueActivityType, type IssueAssignee, type IssueBrief, type IssueCommentAdd, type IssueDetailed, type IssueDocument, type IssueDocumentAssignee, type IssueDocumentFacets, type IssueDocumentFilter, type IssueDocumentList, type IssueDocumentQuery, type IssueFacetsAssignee, type IssueRaw, type IssueReindexDocument, type IssueReindexDocumentBatch, IssueResolution, IssueSortField, type IssueSortableQuery, IssueStatus, type IssueTransition, IssueType, type JiraInstallation, type JiraInstalled, type JwtRestriction, LawsuitRiskStatus, type ListIssues, type ListProjectDocumentList, type ListQualityGate, LocationType, type LoginBody, type MainBranchCommitScanReportsParams, type Member, type MemberId, MemberStatus, type MembersList, type MessageResponse, MfaMethod, type NewManualRepository, type NewOrganization, type NewQualityGate, type NewQualityGateCondition, type NotFoundErrorType, NumberScansFilter, type OAuthConnection, Order, type Organization, type OrganizationList, type OrganizationMembership, type OrganizationMembershipList, type OrganizationQualityProfile, type OrganizationSlug, OrganizationType, type OrganizationUpdate, type PackageLastVersion, type Page, type PageDocument, type PageDocumentList, type PageDocumentQuery, type PageReindexDocument, type PageReindexDocumentBatch, type Pageable, type PageableQuery, type ParsedAlmNamespaceParam, type PaymentHistory, type PaymentHistoryList, type PaymentMethodDetails, type PaymentMethodsList, type PaymentType, type Product, type ProductStaticPrices, type ProductStaticPricesTier, type Project, type ProjectBriefInfo, type ProjectDecryptedToken, type ProjectDocument, type ProjectDocumentFacets, type ProjectDocumentFilter, type ProjectDocumentQuery, type ProjectList, type ProjectNameCompatible, type ProjectNameExists, type ProjectQualityGateData, type ProjectReindexDocument, type ProjectReindexDocumentBatch, type ProjectSettings, type ProjectSettingsAlm, type ProjectSettingsAlmInfo, type ProjectSettingsBase, type ProjectSettingsSite, type ProjectSetupInstructions, ProjectSortField, type ProjectSortableQuery, ProjectType, type PublicAccountInfo, type PullRequestCommitScanReportsParams, type QualityGate, QualityGateAction, type QualityGateCondition, type QualityGateConditionBatch, type QualityGateConditionBrief, type QualityGateConditionBriefBatch, type QualityGateConditions, QualityGateMetric, type QualityGateMetricBatch, type QualityGateMetricConditions, QualityGateMetricInitial, QualityGateMetricSummary, QualityGateOperator, type QualityGateQualityProfile, QualityGateRuleStatus, QualityGateScope, type QualityGateTable, type QualityGateTableCondition, type QualityGateTableRow, type QualityGateWithUsedProjectsData, QualityProfileBuiltInType, type QualityProfileFilter, QualityProfileFilterType, type QualityProfileFilters, type QualityProfileQualityGate, QualityProfileType, type QueryParams, type QueryParamsName, type ReadyScanReport, type RecentDate, type ReconnectRepositoryParams, RegistrationOrigin, type RemoveMember, type RemoveUserPicture, type RenameMainBranch, type RenameQualityGate, type RenameUserBody, type Repository, type RepositoryExistsQuery, RetentionType, type RuleDescriptor, type RuleDescriptorReference, SamlMetadataUriType, type Scan, type ScanArtifact, type ScanArtifactUploadData, type ScanArtifactUploadFormData, type ScanArtifactUploadMeta, type ScanArtifactUploadResult, type ScanArtifactUploadResultData, ScanArtifactUploadStatus, type ScanConsumptionResponse, type ScanDocumentFilter, type ScanDocumentQuery, type ScanIssuePayload, type ScanList, type ScanProcessConfig, type ScanReindexDocument, type ScanReindexDocumentBatch, type ScanRelations, ScanRuleLevel, ScanRuleSeverity, type ScanScore, ScanScoreOutcome, ScanSortField, type ScanSortableQuery, type ScanStats, ScanStatus, type ScanTemplateViolation, type ScanTemplateViolationDto, type ScanTemplateViolationList, type Scans, type ScansConsumption, type ScansConsumptionQuery, type ScoreCell, ScoreCellMetric, type ScriptAuth, type SendEmailMfa, type ServiceHealth, type ServiceVersions, type SetAvatar, type SetDefaultAssignee, type SetIssueTransition, type SignUpBody, type SignUpData, SortOrder, type SsoAuthTokenDto, type SsoAuthUser, SsoEmailCredibilityStatus, SsoProtocolType, type SsoUserInfo, SsoUserLoginMechanism, SsoUserRole, SsoUserType, type StaticPrices, StatusTransition, type StripeCustomerId, type Subscription, SubscriptionBillingCycleType, type SubscriptionInfo, SubscriptionStatus, type SubscriptionTypeInfo, type TagCount, type TagProjectSearch, type TagProjectSet, Tags, type TargetAccountRecord, type TemplateFacets, type TemplatesFacets, TestFramework, type ToggleAutoRenew, type TokenExtendedDetails, type TokenPair, type TopUp, type UpdateMemberPermissions, type UpdateOAuthPart, type UpdateProjectName, type UpdateProjectSettingsDetectTemplateIssues, type UpdateProjectSettingsGateId, type UpdateProjectSettingsGateStatus, type UpdateProjectSettingsIgnoreFixable, type UpdateProjectSettingsPageTemplates, type UpdateProjectSettingsUrlParams, type UpdateProtocol, type UpdateQualityGateCondition, type UpdateSamlPart, type UpgradeSubscriptionBody, UserRegistrationType, UserServiceStatus, type UserSsoConfiguration, type UwOAuthConfiguration, UwOauthProvider, type UwSamlConfiguration, UwSamlProvider, WebInfoOrigin };
